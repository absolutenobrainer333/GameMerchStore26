// UNUSED — superseded by pages/Cart.jsx which has checkout, auth, and context integration
...
const Cart = () => {
  ...
  useEffect(() => {
    setLoading(true);
    Promise.all([
      axiosclient.get("/order/order-detail"),
      axiosclient.get("/order"),
    ])
      .then(([detailRes, orderRes]) => {
        setCartItem(detailRes.data);
        setCart(orderRes.data);
      })
      .catch(() => {
    ...
    return (
      <section className="bg-white py-8 antialiased dark:bg-gray-900 md:py-16">
        <div className="mx-auto max-w-screen-xl px-4 2xl:px-0">
          ...
          <p className="mt-10 text-center text-red-500">{fetchError}</p>
        </div>
      </section>
    );
  }
