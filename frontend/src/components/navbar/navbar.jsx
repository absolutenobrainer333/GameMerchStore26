...
const Navbar = () => {
  ...
  const { cartQuantity, isLoggedIn, logout } = useContext(shopcontext);
    ...
    >
      ...
      <div className="flex-grow hidden lg:flex lg:justify-center lg:items-center">
        <ul className="flex text-[#22e000] text-2xl space-x-4">
          <li className="links p-4"><Link to="/">Home</Link></li>
          <li className="links p-4"><Link to="/phasmo">Phasmophobia</Link></li>
          <li className="links p-4"><Link to="/valo">Valorant</Link></li>
          <li className="links p-4"><Link to="/shop">Shop</Link></li>
        </ul>
      </div>
      ...
      <div className="flex items-center ml-auto gap-8 text-white">
        {isLoggedIn ? (
          <button
            ...
            className="login-button bg-red-500 text-white rounded-lg md:text-xl text-md px-4 py-2 hover:bg-red-600 transition"
          >
            Logout
          </button>
        ) : (
        ...
        <button onClick={() => setIsOpen((prev) => !prev)} className="lg:hidden text-white menu-icon">
          <IoMenu size={40} />
        </button>
      </div>
      ...
      {isOpen && (
        ...
        >
          <ul className="flex flex-col text-right ml-4">
            <li className="links1 p-4"><Link to="/">Home</Link></li>
            <li className="links1 p-4"><Link to="/phasmo">Phasmophobia</Link></li>
            <li className="links1 p-4"><Link to="/valo">Valorant</Link></li>
            <li className="links1 p-4"><Link to="/shop">Shop</Link></li>
          </ul>
        </div>
      )}
    </header>
